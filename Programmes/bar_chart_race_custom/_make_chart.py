...
class _BarChartRace:
    ...
    def __init__(self, df, filename, orientation, sort, n_bars, fixed_order, fixed_max,
                 ...
                 writer, fig, dpi, bar_kwargs, filter_column_colors,logos_path=None):
        ...
        if logos_path is not None:
            for file in os.listdir(logos_path):
                if file.lower().endswith((".png", ".jpg", ".jpeg")):
                    ...
                    try:
                        self.logo_imgs[name] = mpimg.imread(os.path.join(logos_path, file))
                    except:
    ...
    def create_figure(self, dpi):
        ...
        ax.set_facecolor('.9')
        ax.set_title(self.title, size=self.title_size,fontweight="bold")
    ...
    def plot_bars(self, i):
        ...
        if self.logo_imgs:
            for idx, cat in enumerate(cols):
                if cat in self.logo_imgs:
                    # Zoom proportionnel à la hauteur/largeur de la barre
                    if self.orientation == 'h':
                        # Hauteur disponible divisée par le nombre de barres visibles
                        bar_thickness = 1.0 / self.n_bars  
                    else:
                        # Largeur disponible divisée par le nombre de barres visibles
                        bar_thickness = 1.0 / self.n_bars  

                    zoom = bar_thickness * 0.98  # facteur à ajuster

                    image = OffsetImage(self.logo_imgs[cat], zoom=zoom)

                    if self.orientation == 'h':
                        # Horizontal : logo centré dans la barre, légèrement à l'intérieur du bord gauche
                        x = bar_length[idx] * 0.97  # 97% de la longueur de la barre
                        y = bar_location[idx]
                        ab = AnnotationBbox(image, (x, y), frameon=False, box_alignment=(0.5, 0.5))

                    else:
                        # Vertical : logo centré dans la barre, légèrement à l'intérieur du bas
                        x = bar_location[idx]
                        y = bar_length[idx] * 0.97  # 97% de la hauteur de la barre
                        ab = AnnotationBbox(image, (x, y), frameon=False, box_alignment=(0.5, 0.5))

                    # Ajouter et stocker pour suppression à la frame suivante
                    self.ax.add_artist(ab)
                    if not hasattr(self, "_logo_artists"):
                        self._logo_artists = []
                    self._logo_artists.append(ab)
        ...
        if self.period_label:
            ...
            if num_texts == 0:
                # first frame
                self.ax.text(s=s, transform=self.ax.transAxes, **self.period_label,fontweight='bold')
            else:
        ...
        if self.period_summary_func:
            ...
            if 'x' not in text_dict or 'y' not in text_dict or 's' not in text_dict:
                ...
                raise ValueError(f'The dictionary returned from `{name}` must contain '
                                  '"x", "y", and "s"')
            self.ax.text(transform=self.ax.transAxes, **text_dict,fontweight='bold')
        ...
        if self.label_bars:
            ...
            for x1, y1 in zipped:
                ...
                text = self.ax.text(xtext, ytext, text, ha=ha, rotation=rotation, 
                             fontsize=1.5*self.bar_label_size, va=va,fontweight='bold')
    ...
    def anim_func(self, i):
        ...
        if hasattr(self, "_logo_artists"):
            for logo in self._logo_artists:
                logo.remove()
        self._logo_artists = []
...
def bar_chart_race(df, filename=None, orientation='h', sort='desc', n_bars=None, 
                   ...
                   fig=None, dpi=144, bar_kwargs=None, filter_column_colors=False,logos_path=None):
    '''
    ...
    bcr = _BarChartRace(df, filename, orientation, sort, n_bars, fixed_order, fixed_max,
                        ...
                        shared_fontdict, scale, writer, fig, dpi, bar_kwargs, filter_column_colors,logos_path)
